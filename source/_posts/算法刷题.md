    ...
    public:
        vector<int> findAnagrams(string s, string p) {
            ...
            for (int i = 0; i < p_len; ++i) {
          	...
          	// 重载的==判断，维护p_len大小窗口，计算每个字符的个数即可，这里使用的是数组的下标
          	// 这里使用的是数组的下标, 表示个数
...
7. 接雨水
   ...
   public:
       int maxArea(vector<int>& height) {
           ...
           int ans = 0;
           while (l < r) {  // 双指针向内部移动，面积为两边的最短高度乘以宽度
...
8. 打家劫舍
   ...
   只需要考虑三种情况，1. 不包含首尾 2. 包含首元素 3. 包含尾元素。 2和3会包含一，只需要考虑后面两种即可ß
